...
export function enumerate(iterable) {
    if(! (Symbol.iterator in iterable) ) throw new TypeError(
...
export function chain(...iterables) {
    ...
    for(const it of iterables) {
        if(! (Symbol.iterator in it)) {
            throw new TypeError(`${i}th value does not appear iterable ${JSON.stringify(it) }`);
        }
    }
...
export function product(...iterables) {
    ...
    for(const [i, iterable] of iterables.entries()) {
        if (! (Symbol.iterator in iterable)) throw TypeError(
